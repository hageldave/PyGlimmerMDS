...
def execute_glimmer_pd(
    ...
    ----------
    pd : np.ndarray
        pair-wise distances, i.e., the dissimilarity matrix containing the pair-wise distances. (square 2D array)
    ...
    """
    if len(pd.shape) != 2 or pd.shape[0] != pd.shape[1]:
        raise ValueError(f"dissimilarity matrix is not square but of shape {pd.shape}")
    if rng is None:
        rng = np.random.default_rng()
    if initialization is None:
        if target_dim is None:
            target_dim = 2
        norms = np.mean(pd, axis=1)
        initialization = rng.random((pd.shape[0], target_dim))-.5
    ...
    for level in range(n_levels-1, -1, -1):
        ...
        for iter in range(max_iter):
            ...
            new_neighbor_candidates = __rand_indices_noduplicates_on_rows(
                ...
                neighbor_set_size*2,
                rng)
            __update_neighbors(current_neighbors, new_neighbor_candidates, current_pd, current_pd, neighbor_set_size)
...
def __update_neighbors(curr_neighbors, new_randoms, pd, neighbor_pd, k):
...
def layout(pd: np.ndarray, embedding: np.ndarray, forces: np.ndarray, neighbors: np.ndarray, start=0, end=None, alpha=1.0):
    # for each point get neighbor points and compute forces
    if end is None:
        end = pd.shape[0]
    return __compute_forces_and_layout(pd, embedding, forces, neighbors, start, end, alpha)


def __compute_forces_and_layout(pd: np.ndarray, embedding: np.ndarray, forces: np.ndarray, neighbors: np.ndarray, start:int, end:int, alpha=1.0):
